...
test("authenticated onboarding completes and returns to the requested private route", async ({
  ...
  await expect(page.getByRole("heading", { name: "Tell us what progress means to you." })).toBeVisible();
  await page.getByLabel("What should we call you?").fill("Staging QA Learner");
  await page.getByLabel("Age group").selectOption("25_34");
  await page.getByLabel("What do you want to achieve first?").fill("Complete reliable SkillUp staging certification");
...
test("onboarding validation prevents incomplete submission", async ({ page }) => {
  ...
  await page.getByRole("button", { name: "Save and continue" }).click();
  expect(await page.getByLabel("What should we call you?").evaluate((element) => element.validity.valid)).toBe(false);
  expect(await page.getByLabel("What do you want to achieve first?").evaluate((element) => element.validity.valid)).toBe(false);
});
...
test("onboarding reports a safe API/network failure without losing the form", async ({ page }) => {
  ...
  await page.getByRole("button", { name: "Save and continue" }).click();
  await expect(page.getByText("We could not save your profile. Check your connection and try again.")).toBeVisible();
