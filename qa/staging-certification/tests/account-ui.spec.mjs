...
test("learner can change and restore a privacy preference through the browser", async ({
  page,
  request,
}) => {
...
test("learner can download the bounded private JSON export from the account UI", async ({
  page,
}) => {
  await page.goto("/en/account");
  ...
  await expect(
    page.getByText("Your export was prepared and downloaded on this device."),
  ).toBeVisible();
});

test("account deletion can be scheduled and cancelled through the browser cooldown flow", async ({
  page,
  request,
}) => {
